...
public final class TypeHandlerRegistry {
  ...
  /**
   * key    是javaType java对象的数据类型
   * value  是一个Map，map中key为jdbcType数据库数据类型，value为TypeHandler
   */
  ...
  /**
   *
   * @param jdbcType 数据库的字段类型
   * @param handler  处理的TypeHandler类
   */
   ...
   *   java type + handler
   *
   * @param javaType java对象的字段类型
   * @param typeHandler 处理的TypeHandler类
   * @param <T>
   */
  ...
  private <T> void register(Type javaType, TypeHandler<? extends T> typeHandler) {
    ...
    if (mappedJdbcTypes != null) {
      //mappedJdbcTypes.value() 返回是数组，for循环判断处理
