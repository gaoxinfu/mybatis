...
public class XMLStatementBuilder extends BaseBuilder {
  ...
  /**
   * 例如
   * <select resultMap="BaseResultMap" parameterType="com.gaoxinfu.demo.open.source.mybatis.db.model.BlogExample" id="selectByExample">
   *    <if test="distinct">
   *       distinct
   *     </if>
   ...
   * @param id
   * @param databaseId 这个是当前节点上配置的databaseId类型 (oracle 或者mysql 或者db2等等)
   * @param requiredDatabaseId 其实就是我们配置的数据库类型:oracle 或者mysql 或者db2等等,这个相当于需要的数据库类型
   *
   * 要么两个databaseId 和requiredDatabaseId 都不配置，
   * 要么配置了两个必须一样
   * @return
   */
